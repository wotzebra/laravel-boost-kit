...
- Use `$response->assertValidRequest()` (or `$response->assertInvalidRequest()`), `$response->assertValidResponse($statusCode)` (or `$response->assertInvalidResponse($statusCode)`) in each test to validate the API endpoint against the OpenAPI schema. If there in no response schema defined for a specific status code in the OpenAPI specification, then do NOT use `$response->assertStatus($statusCode)` and ask the user to add the response schema for that statuscode to the OpenAPI specification.
