...
- Create a Controller method that corresponds to the endpoint. The Controller should have thin logic, delegating complex operations to Actions. The controller method name should reflect the action (e.g., `index`, `show`, `store()`, `update()`, `destroy()`) and use route model binding where applicable. If the endpoint requires authentication, use `#[CurrentUser] User $user` to access the authenticated user instead of `$request->user()`.
- Create a Form Request class for validating incoming requests based on the OpenAPI schema. Use array-based validation rules and include helper methods for computed values.
